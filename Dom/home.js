// отрисовать 20 карточек на странице не используя html с помощью массива по ссылке
// Используте цикл или метод перебирания массива

// здесь пишем так или let arr = ([передаем данные которые перебрать надо])-let arr пишем до перебирания в конце пишем аргументы createBox()
...
// input.addEventListener("input", () => {           надо дописать 
//     console.log("hello");
// })
...
// }
// ------------------------------------------------------------
// нужно взять квадрат и при ползунке, когла первый двигается пусть левая часть квадрата округляется и т.д.
// let box = document.querySelector(".box")
...
// for(let i = 0; i < inputs.length; i++){
//     inputs[i].addEventListener("input", (e) => {
//        box.style.borderRadius= `${leftTop.value}px ${rightTop.value}px ${rightBottom.value}px ${leftBottom.value}px`
//     })
// }
// ------------------------------------------------------------
// у нас есть инпут,пишем в инпуте и бокс, квадарат должен перекрашиваться в тот цвет котрый напиали в инпуте
// let box = document.querySelector(".box")
// let text = document.querySelector("#changecolor")
...
/* 1) у вас есть квадрат то в ползунке первом маржин сверху, второй ползунок мажин справа и т.квадрат, 4 ползунка
2) в инпуте пишете емайл, потом пассворд, при написании емайла просто текст должен появиться снизу, при нажатии на энтер */
